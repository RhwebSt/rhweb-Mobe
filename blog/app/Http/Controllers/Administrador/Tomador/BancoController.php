                            ...
                            if ($tomadors) {
                                ...
                                foreach ($rublicas as $key => $rublica) {
                                    for ($i=0; $i < 2; $i++) {
                                            ...
                                            if (strpos(strtoupper($file->getClientOriginalName()), 'TABELA') !== false) {
                                                $dados = file($file);
                                                foreach ($dados as $i => $linha) {
                                                    if ($tomador['matricula'] == str_replace("  ", "",substr(utf8_encode($linha), 7, 6))) {
                                                        
                                                        if (strpos(strtoupper(str_replace("  ", "",substr(utf8_encode($linha), 16, 39))), strtoupper($rublica->rsdescricao)) !== false) {
                                                            // dd(str_replace("  ", "",substr(utf8_encode($linha), 7, 6)),$tomador['matricula']);
                                                            // dd(strtoupper(str_replace("  ", "",substr(utf8_encode($linha), 16, 39))));
                                                            // dd(number_format((float)str_replace("  ", "",substr(utf8_encode($linha), 65, 8)), 2, ',', '.'));
                                                            $dadostabelapreco = [
                                                                'ano' => date('Y-m',strtotime(str_replace("  ", "",substr(utf8_encode($linha), 0, 6)))),
                                                                'rubricas' => $rublica->rsrublica,
                                                                'descricao' => $rublica->rsdescricao,
                                                                'status' => '',
                                                                'valor' => number_format((float)str_replace("  ", "",substr(utf8_encode($linha), 65, 4)), 2, ',', '.'),
                                                                'valor__tomador' =>number_format((float)str_replace("  ", "",substr(utf8_encode($linha), 65, 4)), 2, ',', '.'),
                                                                'empresa' => $empresa['empresa'],
                                                                'tomador' => $tomadors['id']
                                                            ];
